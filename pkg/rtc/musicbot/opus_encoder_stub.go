...
// build cannot link. A μ-law source plays unaffected, an opus source
// even more so (its packets pass through with no codec at all); a
// linear PCM source fails when a call tries to prepare it, with the
// error below saying why.

import "errors"

// errNoOpus explains the limitation.
var errNoOpus = errors.New("musicbot: playing a linear pcm source needs the opus encoder — build with cgo and libopus available (CGO_ENABLED=1); mu-law and opus sources play without it")
