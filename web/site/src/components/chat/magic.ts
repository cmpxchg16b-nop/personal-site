...
const DELETE_RE =
  /^\/magic a90926e3-c768-45b7-ab93-4709c5f4aa91 (\S+)$/;
const AMEND_TEXT_RE =
...
// (a file transfer's id, name and MIME type) from the conversation
...
export function buildMagicControl(
  ...
  switch (command.kind) {
    ...
    case "amend-file-transfer": {
      const target = history.find((m) => m.id === command.targetMessageId);
      if (target === undefined || target.type !== "file-transfer-status") {
        return null;
      }
      return {
        subtype: "amend",
